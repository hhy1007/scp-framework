...
public class PostgreSqlHelper implements DialectHelper {
  ...
  private static final String TOTAL_RECORD_NUM = "total_record_num";
	...
	public static String getCountString(String querySelect) {
		...
		if (select.toLowerCase().indexOf("select distinct") != -1
			...
			return new StringBuffer(querySelect.length())
					...
					.append(TOTAL_RECORD_NUM).append(" from (")
					.append(querySelect.substring(0, orderIndex))
					.append(" ) t").toString();
		} else {
			return new StringBuffer(querySelect.length())
					.append("select count(1) ")
					.append(TOTAL_RECORD_NUM).append(" ")
	...
	private static int getLastOrderInsertPoint(String querySelect) {
		...
		if (orderIndex == -1
				|| !isBracketCanPartnership(querySelect.substring(orderIndex,
						querySelect.length()))) {
			throw new CommonException(null, "My SQL 分页必须要有Order by 语句!");
		}
		return orderIndex;
	}
